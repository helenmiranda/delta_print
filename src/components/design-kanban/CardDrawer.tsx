// Drawer lateral de detalhes de um card do Design Kanban
// Abre ao clicar em qualquer card — slide da direita, 480px em desktop, tela cheia no mobile
import { useState, useEffect, useRef, useCallback } from 'react';
import {
  X, User, Phone, Calendar, ChevronDown, Tag, Plus,
  AlertCircle, Clock, UserCheck, AlignLeft, Archive, Trash2,
} from 'lucide-react';
...
interface CardDrawerProps {
  ...
  onClose: () => void;
  onCardUpdated: () => void;
  // Callback chamado quando o card é arquivado (para remover do board sem reload)
  onCardArchived?: (cardId: string) => void;
  // Callback chamado quando o card é deletado permanentemente
...
// Gera iniciais de um nome para avatar
function getInitials(name: string | null): string {
  if (!name) return '?';
  return name.split(' ').slice(0, 2).map((n) => n[0]).join('').toUpperCase();
}

// Gera cor de avatar por email
...
// Campo editável inline — vira input ao clicar
...
function Divider() {
  return <div className="my-5" style={{ borderTop: '1px solid #e2e8f0' }} />;
}
...
export default function CardDrawer({
  ...
  // Controle do modal de confirmação de exclusão permanente
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [archiving, setArchiving] = useState(false);

  // Labels disponíveis para adicionar ao card
  ...
  // Animação de entrada: aguarda um tick para aplicar a classe de visibilidade
  useEffect(() => {
    if (cardId) {
  ...
  // Buscar dados completos do card quando o drawer abre
  const fetchCard = useCallback(async () => {
    ...
    if (error) {
      console.error('[CardDrawer] fetchCard error:', error);
      setLoading(false);
      return;
    }

    if (data) {
      // Buscar labels vinculadas ao card
  ...
  // Salvar campo direto no Supabase e atualizar estado local
  async function saveField(field: Partial<DesignCard>) {
    ...
    if (error) {
      console.error('[CardDrawer] saveField error:', error);
    } else {
      onCardUpdated();
    }
  }

  // Salvar mudança de coluna com log de atividade
  async function saveColumnChange(newColumnId: string) {
    ...
    await supabase.from('design_activity_log').insert({
      ...
      actor_name: userName,
      action: 'card_moved',
      details: { from_column: oldColumnId, to_column: newColumnId },
    });
  }
  ...
  async function addLabel(label: DesignLabel) {
    ...
    if (!error) {
      ...
      await supabase.from('design_activity_log').insert({
        ...
        actor_name: userName,
        action: 'label_added',
        details: { label_id: label.id, label_name: label.name },
      });
    }
  ...
  // Salvar descrição com debounce de 1 segundo
  ...
  // Arquivar card: marca is_archived = true, registra log e fecha o drawer
  ...
  // Deletar card permanentemente — cascade remove checklist, comentários e anexos
  ...
  // Coluna atual do card
  const currentColumn = columns.find((c) => c.id === card?.column_id);

  // Labels ainda não vinculadas ao card
  const availableLabels = allLabels.filter(
    (l) => !card?.labels?.some((cl) => cl.id === l.id)
  );

  // Mapa de id→título para o componente de log de atividades traduzir IDs de colunas
  const columnMap: Record<string, string> = Object.fromEntries(
    columns.map((c) => [c.id, c.title])
  );

  // Verificar se a data de entrega está atrasada
  ...
  return (
    <>
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
            ...
            <p className="text-sm text-gray-500 mb-5">
              Esta ação é permanente e não pode ser desfeita. Todos os comentários,
              checklist e anexos serão removidos.
            </p>
      ...
      <div
        className="fixed right-0 top-0 h-full z-50 flex flex-col bg-white shadow-2xl overflow-hidden"
        style={{
      ...
      >
        {loading || !card ? (
          ...
          <div className="flex flex-col h-full overflow-hidden">
            {/* ── HEADER FIXO ─────────────────────────── */}
            <div className="flex-shrink-0 px-5 pt-5 pb-4" style={{ borderBottom: '1px solid #e2e8f0' }}>
              {/* Botão fechar */}
            ...
            <div className="flex-1 px-5 py-5 overflow-y-auto">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {card.cliente_telefone && (
                      <a
                        href={`tel:${card.cliente_telefone.replace(/\D/g, '')}`}
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full appearance-none px-3 py-2 pr-8 rounded-lg border text-sm outline-none transition-colors bg-white"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCard((prev) =>
                        prev ? { ...prev, assigned_to_name: e.target.value } : prev
                      )
                    }
              ...
              {/* SEÇÃO: SOLICITADO POR (somente leitura) */}
              <div
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  <div
                    ...
                    style={{ background: getAvatarColor(card.requested_by), fontSize: '11px' }}
                  >
                    {getInitials(card.requested_by_name)}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">
                      {card.requested_by_name || 'Desconhecido'}
                    </p>
                    <p className="text-xs text-gray-500 truncate">{card.requested_by}</p>
                  </div>
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  {/* Chips de labels vinculadas */}
                  {(card.labels ?? []).map((label) => (
                    <span
                      key={label.id}
                      className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-white font-medium"
                      style={{ background: label.color, fontSize: '12px' }}
                    >
                      {label.name}
                      <button
                  ...
                  {/* Botão para abrir dropdown de labels */}
                  <div className="relative">
                    <button
              ...
              <div className="mb-5">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors resize-none leading-relaxed"
                  style={{ borderColor: '#e2e8f0', fontSize: '13px' }}
                  onFocus={(e) => (e.target.style.borderColor = '#3D4465')}
              ...
              {/* SEÇÃO: ANEXOS — upload por clique ou drag & drop */}
              <div className="mb-5">
                <CardAttachments
                  cardId={card.id}
                  userEmail={userEmail}
              ...
              {/* SEÇÃO: LOG DE ATIVIDADES — timeline com realtime */}
              <div className="mb-5">
                <CardActivityLog cardId={card.id} columnMap={columnMap} />
              </div>

              {/* Espaçamento extra para não ocultar conteúdo atrás do rodapé fixo */}
              <div className="h-2" />
            </div>

            {/* ── RODAPÉ FIXO: AÇÕES DO CARD ────────── */}
            <div
              className="flex-shrink-0 flex items-center gap-3 px-5 py-3"
              style={{ borderTop: '1px solid #e2e8f0', background: '#fafafa' }}
            >
              {/* Arquivar card — remove do board mas mantém os dados */}
              <button
                onClick={handleArchive}
                disabled={archiving}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-gray-500 hover:bg-gray-100 transition-colors disabled:opacity-40 border border-gray-200"
              >
                <Archive className="w-4 h-4" />
                {archiving ? 'Arquivando...' : 'Arquivar'}
              </button>

              {/* Deletar card permanentemente — abre modal de confirmação */}
              <button
                onClick={() => setShowDeleteConfirm(true)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50 transition-colors border border-red-200 ml-auto"
              >
...
// Componente separado para o título editável inline do header
function TitleEditor({
  ...
  return (
    <h2
      onClick={() => setEditing(true)}
      className="font-bold text-gray-800 text-lg leading-snug cursor-pointer hover:bg-gray-50 rounded px-1 py-0.5 transition-colors"
